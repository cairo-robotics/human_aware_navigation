...
function main()
    ...
    Initialize vehicle and vehicle_params and vehicle_sensor
    Initialize humans and human_params
    Initialize sim_obj
    Initialize exp_values from config file
    Initialize first_action


    =#
    config_rng = MersenneTwister(11)
    ...
    env_humans, env_humans_params = generate_humans(env,veh,1.0,list_human_goals,200,config_rng)
    ...
    exp_details = experiment_details(
        ...
        false, #=vehicle_ran_into_boundary_wall_flag::Bool=#
        false, #=vehicle_ran_into_obstacle_flag::Bool=#
        false, #=vehicle_reached_goal_flag::Bool=#
        false, #=experiment_success_flag::Bool=#
        list_human_goals, #=human_goal_locations::Array{location,1}=#
        OrderedDict(), #=observed_beliefs::OrderedDict=#
    ...
    current_sim_obj = simulator(env,veh,veh_params,veh_sensor_data,env_humans,env_humans_params,0.1)
    ...
    while(!stop_simulation!(current_sim_obj,current_time_value,exp_details))
        ...
        current_vehicle_speed = clamp(current_vehicle_speed+next_action.delta_speed, 0.0, exp_details.max_vehicle_speed)
