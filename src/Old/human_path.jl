...
function human_actions(max_delta_angle, min_delta_angle_difference)
...
function get_new_human_position(human,action,time_duration)
...
function hybrid_astar_search(world, vehicle, vehicle_params, vehicle_actions, nearby_humans, planning_details)
    ...
    start_node = GraphNode(vehicle.x,vehicle.y,vehicle.theta,0.0,calculate_heuristic_cost(vehicle.x,vehicle.y,vehicle.theta,vehicle_params.goal.x,vehicle_params.goal.y,world),
    ...
    while(length(open) != 0)
        ...
        for delta_heading_angle in vehicle_actions
            ...
            new_node = GraphNode(new_x,new_y,new_theta,g,h,steering_angle,discrete_x,discrete_y,discrete_theta,current_node,current_node.time_stamp+planning_details.one_time_step)
